...
var lastAdded = new Dog(defaultData);
...
//function to find a specific Dog on request. Express functions always receive the request and the response.
var readDog = function(req, res) {
    ...
    //Call the static function attached to DogModels. This was defined in the Schema in the Model file. 
...
var searchName = function(req,res) {
    ...
    Dog.findByName(req.query.name, function(err, doc) {
        ...
        if(!doc) {
            return res.json({error: "No dogs found by the name of " + req.query.name});
        }
...
var updateLast = function(req, res) {
    ...
    lastAdded.save(function(err) {
        ...
        return res.json({name:lastAdded.name, age: lastAdded.age});
    });
};
